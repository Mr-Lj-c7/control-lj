#include "connect_lon_controller_test.h"

namespace control{
 
// 加载数据
LocalizationPb ConnectLonControllerTest::LoadLocalizationPb (
...
ChassisPb ConnectLonControllerTest::LoadChassisPb(
    const std::string& filename) const {
    ...
    if (!reader_2.parse(file, root_2)) {
        std::cerr << "Failed to parse JSON data in 2_chassis file." << std::endl;
        return chassis_pb;
    }
    // 车辆状态
...
PlanningTrajectoryPb ConnectLonControllerTest::LoadPlanningTrajectoryPb(
    const std::string& filename) const {
...
// 检核结果
bool ConnectLonControllerTest::CustomExpectedResultWithMessage(
...
char* ConnectLonControllerTest::Run() {
    injector_ = std::shared_ptr<DependencyInjector>
                 (new DependencyInjector());
    // load datas
    auto localiation_file = std::string(data_path) + "1_localization.json";
    auto chassis_file = std::string(data_path) + "1_chassis.json";

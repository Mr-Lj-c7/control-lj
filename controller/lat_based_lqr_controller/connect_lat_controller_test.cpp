...
namespace control{
// 定位
LocalizationPb ConnectLatControllerTest::LoadLocalizationPb(
...
// 规划
PlanningTrajectoryPb ConnectLatControllerTest::LoadPlanningTrajectoryPb(
    const std::string& filename) const {
    ...
    if (relativate_time_)
        planning_trajectory_pb.header.timestamp_sec = timestamp_;
    return planning_trajectory_pb;
}

// 底盘
ChassisPb ConnectLatControllerTest::LoadChassisPb(
    const std::string& filename) const {
...
// 校验
bool ConnectLatControllerTest::CustomExpectedResultWithMessage(
...
char* ConnectLatControllerTest::Run() {

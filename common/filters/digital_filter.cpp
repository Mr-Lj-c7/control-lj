...
DigitalFilter::DigitalFilter(
    const std::vector<double> &denomintors,
...
void DigitalFilter::SetCoefficients(
...
double DigitalFilter::Compute(const std::deque<double> &values, 
    ...
    if (!(coeff_start <= coeff_end && coeff_end < cofficients.size())) {
        std::cerr << "cofficients index out of range" << std::endl;
    }
    if (!(coeff_end - coeff_start + 1 == values.size())) {
        std::cerr << "cofficients size != values size" << std::endl;
    }
    double sum = 0.0;
    auto i = coeff_start;
